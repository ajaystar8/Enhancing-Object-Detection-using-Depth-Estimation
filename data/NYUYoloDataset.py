"""
This module defines the NYUYoloDataset class for loading the NYU Depth dataset
for YOLO-based object detection. It includes methods for handling RGB, HHA, and
fusion images, extracting bounding boxes, applying transformations, and generating
YOLO-specific target labels.

The dataset is designed for training YOLO object detection models on the NYU Depth
dataset with optional transformations, label generation, and custom target category
support. The dataset supports different modes including RGB, HHA, and fusion of RGB
and depth data.
"""
...
class NYUYoloDataset(Dataset):
	def __init__(self, mat_file, hha_dir, anchors, image_size=416, S=None, C=40, apply_transforms=True, indices=None,
		...
		"""
		Custom dataset class for loading images and corresponding labels from the NYU dataset, preparing them
		for YOLO training. The dataset class processes data from both RGB images and HHA depth images,
		applying necessary transformations for training. Bounding boxes are extracted and formatted for YOLO training.
		The dataset supports label generation in YOLO format for training purposes.
