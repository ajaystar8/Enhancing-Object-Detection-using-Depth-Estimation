...
def getRMatrix(yi, yf):
    ...
    if abs(phi) > 0.1:  # If the angle is significant
        phi = phi * (np.pi / 180)  # Convert to radians

        # Create skew-symmetric matrix s_hat from the axis vector ax
        s_hat = np.array([[0, -ax[2], ax[1]],
                          [ax[2], 0, -ax[0]],
                          [-ax[1], ax[0], 0]])

        # Rodrigues' rotation formula to calculate the rotation matrix
        R = np.eye(3) + np.sin(phi) * s_hat + (1 - np.cos(phi)) * np.dot(s_hat, s_hat)
    else:
        R = np.eye(3)  # Identity matrix if the angle is small
